...
const pagina = data.value.pagina || 1

const searchTerm = defineModel();

const filterItems = (filter) => {
    const pagina = data.value.pagina || 1
    data.value.searchTerm = pagina

    const items = [];
    data.value.result.forEach(function (item, index) {
        if (filter == "" || item.dataset.toLowerCase().includes(searchTerm.toLowerCase()) ) {
            items.push(item);
        };
    });
    
    const fromItem = (items.pagina + step) * items.itemPerPag;
    const toItem = fromItem + items.itemPerPag ? (fromItem + items.itemPerPag) < items.length : items.length;
